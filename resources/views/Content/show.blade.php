    ...
    <section>
        <div class="container py-4">
            ...
            @if(auth()->user()->hasRole('student'))
                <form class="default">
                    @csrf
                    <textarea name="text" class="notepad py-md-4" rows="5" required>{{ auth()->user()->notes()->where('content_id', $content->id)->first()->text ?? '' }}</textarea>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            $(".save").click(function(e){
                ...
                console.log(CSRF_TOKEN, text, content);
                $.ajax({
                    url: '/content/'+content+'/note',
                    type: 'POST',
